...
Запросы к плагину **сериализуются**: он однопоточный, и параллельные вызовы
раньше сгорали по таймауту, пока рендерилась крупная нода. Теперь они встают в
очередь (`GET /render-queue` показывает стадию и длину), таймаут настраивается
через `&timeout=<сек>` (по умолчанию 90), а при неудаче приходит `504` со
стадией `sent`/`rendering`/`lost` — видно, задание не дошло или не успело.
