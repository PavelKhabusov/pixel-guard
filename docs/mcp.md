...
| `pixel_guard_measure` | live page (headless Chromium): every visible element matching `selector` → rect, computed styles, effective inset, parent flex/grid gap and the `step` to the previous match; `sources: true` adds the rule that WON the cascade per property (`value ← selector (stylesheet)`, like DevTools Styles); `cascade: true` lists EVERY competing rule, winner first, losers with their files (who overrides what); hidden ones counted separately (`include_hidden`); `fresh` reloads | site |
| `pixel_guard_probe` | "click and tell": runs `steps` and reports XHR/fetch requests (method, url, POST body, status, ms), console errors / uncaught exceptions, DOM changes (element count, height, added/removed nodes) — finds "click ✓, zero requests" | site |
| `pixel_guard_shot` | screenshot of `selector` or the page after `prepare`; `freeze: ["admin-ajax.php"]` aborts matching requests to capture a skeleton / loading state; inline image or `save_to` | site |
...
On start both servers (`mcp.mjs`, `ingest.mjs`) kill orphaned Playwright browsers left by a killed parent, and SIGTERM/SIGINT/SIGHUP close the browser before exit (`server/lib/lifecycle.mjs`).
