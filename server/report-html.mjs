...
export function renderHtml(report) {
  const rows = report.nodes.map((n) => {
    const badge = { pass: '✓', failed: '✗', missing: 'нет в DOM', skip: 'skip', 'map-error': 'нода не найдена', absent: 'нет в макете страницы' }[n.status];
    const diffs = (n.diffs ?? [])
...
<style>
  ...
  section.failed{border-left-color:#c98b8b} section.missing,section.map-error{border-left-color:#d4b483} section.absent{border-left-color:#4a5766;opacity:.65}
