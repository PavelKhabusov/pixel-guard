...
- Все страницы редизайна в `pages.json`; batch-прогон `pixel-guard run --all`.
- `report.html` v2: дерево страницы, фильтры, скрин ноды из Figma рядом со значениями.
- Команды в `commands-list.json` проекта (запуск сервера, прогоны) — под кнопки
  Commands Extension.

### Фаза 3 — визуальный уровень
- PNG frame'ов из `exportAsync()` (уже в снапшоте) + fullPage-скриншоты Playwright.
- pixelmatch: diff-картинка + процент по странице и по каждому заматченному блоку
...
- Генерация CSS-патчей из диффов (предложение правок, не автоприменение).
